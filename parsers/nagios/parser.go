...
func (p *NagiosParser) ParseLine(line string) (data.Metric, error) {
...
func (p *NagiosParser) Parse(buf []byte) ([]data.Metric, error) {
	...
	for _, unParsedPerfs := range unParsedPerfs {
		...
		// Create metric
		metric, err := data.NewMetric(fieldName, tags, fields, time.Now().UTC())
		if err != nil {
			return nil, err
		}
		// Add Metric
