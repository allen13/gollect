...
func (v *ValueParser) Parse(buf []byte) ([]data.Metric, error) {
	...
	metric, err := data.NewMetric(v.MetricName, v.DefaultTags,
		fields, time.Now().UTC())
	if err != nil {
		return nil, err
	}

	return []data.Metric{metric}, nil
}

func (v *ValueParser) ParseLine(line string) (data.Metric, error) {
