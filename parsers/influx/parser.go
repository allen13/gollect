...
// successfully.
func (p *InfluxParser) Parse(buf []byte) ([]data.Metric, error) {
	// parse even if the buffer begins with a newline
	buf = bytes.TrimPrefix(buf, []byte("\n"))
	points, err := models.ParsePoints(buf)
	metrics := make([]data.Metric, len(points))
	for i, point := range points {
		...
		metrics[i], _ = data.NewMetric(point.Name(), tags,
			point.Fields(), point.Time())
	}
	return metrics, err
}

func (p *InfluxParser) ParseLine(line string) (data.Metric, error) {
